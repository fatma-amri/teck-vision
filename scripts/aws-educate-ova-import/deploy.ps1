param(
    ...
    [string]$BucketName = "ctf-tekup-educate",
    [string]$BucketPrefix = "educate_ovas/",
    [string]$TableName = "ctf-educate-ova-imports",
    [string]$FunctionName = "ctf-educate-ova-importer",
    [string]$LambdaRoleName = "ctf-educate-ova-importer-lambda-role",
    [string]$VmImportRoleName = "vmimport",
    [string]$ChallengePrefix = "edu",
    [string]$ScheduleName = "ctf-educate-ova-importer-reconcile",
    [switch]$SkipSchedule
)
...
function Ensure-VmImportRole {
    ...
    if ($ExitCode -ne 0) {
        aws iam create-role --role-name $VmImportRoleName --assume-role-policy-document "file://$TrustPath" | Out-Null
    }
    ...
    aws iam put-role-policy `
        ...
        --policy-name "$VmImportRoleName-educate-policy" `
    ...
    Write-JsonFile $PolicyPath @{
        ...
        Statement = @(
            @{
                ...
                Resource = @("arn:aws:s3:::$BucketName", "arn:aws:s3:::$BucketName/$BucketPrefix*")
            },
            @{
                Effect = "Allow"
...
function Build-Package {
    ...
    Compress-Archive -Path (Join-Path $ScriptDir "lambda_function.py") -DestinationPath $PackagePath -Force
}

function Ensure-Lambda {
    param([string]$RoleArn)
    $EnvVars = "Variables={TABLE_NAME=$TableName,CHALLENGE_PREFIX=$ChallengePrefix,IMPORT_DESCRIPTION_PREFIX=Educate OVA import}"
    ...
    if ($ExitCode -ne 0) {
        ...
        aws lambda update-function-code --function-name $FunctionName --zip-file "fileb://$PackagePath" --region $Region | Out-Null
...
function Configure-Schedule {
    ...
    Write-JsonFile $TargetsPath @(@{ Id = "EducateOvaImportReconcileLambda"; Arn = $FunctionArn; Input = '{"source":"eventbridge.reconcile"}' })
...
Write-Host "Educate OVA uploads: s3://$BucketName/$BucketPrefix"
